...
if [[ 10#$responseCode -eq 204 ]]; then # Http 204 No Content means that there are no changes
  ...
  echo "remoteChanges=no" >> $GITHUB_ENV
  ...
  echo "remoteChanges=yes" >> $GITHUB_ENV
  echo "lastestDeploymentId=$latestDeploymentId" >> $GITHUB_ENV
  echo "remoteChangeFile=git-patch.diff" >> $GITHUB_ENV
else
